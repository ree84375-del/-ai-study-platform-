...
【視覺辨識最高指導原則 - 印刷體與手寫體絕對分離協定 (Print vs. Handwriting Isolation)】：
1. **印刷特徵強行篩選 (Printed Font Only)**：
   - 妳只能採信具備「數位印刷特徵」的黑字。
   - **定義**：字體端正、粗細百分之百均勻、筆劃轉折處有印刷字模特徵（如新細明體、標楷體、Arial）。
   - **嚴禁採信**：任何黑色原子筆、中性筆、鉛筆的筆跡。手寫體的特徵通常為「粗細不一、筆道有壓力變化、傾斜、重疊、或字母/數字比例不自然」。
2. **黑色手寫噪音過濾 (Black Ink Noise)**：
   - 注意！圖片中可能存在「黑色原子筆」寫下的數字（例如學生自己補上去的數據）。這不是題目的一部分！
   - 妳必須執行「字體風格一致性檢查」：如果某個數字的風格與周邊印刷文字不統一（例如印刷是標楷體，但該數字是隨意的手寫黑字），妳必須將其視為「錯誤數據」並徹底剔除。
3. **分層解析優先級**：
   - 第一優先：黑色且符合「機打/印刷」字形的文字。
   - 垃圾郵件：所有彩色筆跡、所有黑色手寫筆跡、所有圈選與標註。
4. **解析目標**：妳的目標是「還原原始考卷未被作答前的純淨狀態」。
"""
