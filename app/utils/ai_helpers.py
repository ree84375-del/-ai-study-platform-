...
【視覺辨識最高指導原則 - 色彩與字型強行隔離協定 (Chromatic Masking & Font Isolation)】：
1. **深度色彩過濾 (Total Color Masking)**：
   - 妳必須在分析前將圖片進行「邏輯上的去色處理」。
   - **絕對無視**：所有「紅色、藍色、綠色、淺灰色、鉛筆質感」的筆跡。
   - **範例**：若印刷的 (1, -2) 上方被紅筆改寫成 2 或 4，妳必須「殺掉」紅色像素，只看底層模糊但規整的黑色 1 與 -2。
2. **字型特徵判別 (Print vs. Script)**：
   - **印刷層 (Fact)**：只採納字體端正、粗細均勻、符合新細明體 (MingLiU)、標楷體 (KaiTi) 或常見機打字型的「黑色筆劃」。
   - **手寫層 (Spam/Noise)**：任何筆劃粗細不一、傾斜、重疊、或帶有連筆動作的手寫體（無論什麼顏色）均視為「雜訊」，嚴禁讀取其中的數字。
3. **數字與代數絕對校對**：
   - 區分 b/6, l/1, o/0, q/9, z/2。
   - 嚴禁因為紅筆劃過就把 `^2` 看成 `√`。
4. **解析優先權**：印刷黑字 > 影印殘影 > (拋棄) 任何彩色內容。
"""
